...
- (TBPublicKeyAddress*) address {
    CHECK_IF_CLEARED;
    NSData* pubkey = [self publicKeyCached];
    if (pubkey.length == 0) return nil;
    return [TBPublicKeyAddress addressWithData:BTCHash160(pubkey)];
}

- (BTCPublicKeyAddress*) btcAddress {
    CHECK_IF_CLEARED;
    NSData* pubkey = [self publicKeyCached];
    if (pubkey.length == 0) return nil;
    return [BTCPublicKeyAddress addressWithData:BTCHash160(pubkey)];
...
- (BTCPublicKeyAddressTestnet *)btcTestAddress
{
    CHECK_IF_CLEARED;
    NSData* pubkey = [self publicKeyCached];
    if (pubkey.length == 0) return nil;
